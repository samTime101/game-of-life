  ...
  while (!WindowShouldClose()) {
      ...
      for (int i = 0; i < ROWS; i++) {
        for (int j = 0; j < COLS; j++) {
          ...
          int life = mat[up][left] + mat[up][j] + mat[up][right] +
                     mat[i][left] + mat[i][right] + mat[down][j] +
                     mat[down][left] + mat[down][right];
    ...
    for (int row = 0; row < ROWS; row++) {
      for (int col = 0; col < COLS; col++) {
        if (mat[row][col] != 0) {
          DrawRectangle(col * CELL_WIDTH, row * CELL_HEIGHT, CELL_WIDTH,
                        CELL_HEIGHT, BLACK);
        }
        DrawRectangleLines(col * CELL_WIDTH, row * CELL_HEIGHT, CELL_WIDTH,
                           CELL_HEIGHT, GRAY);
      }
    }
    EndDrawing();
